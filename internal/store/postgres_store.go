...
func (s *PostgresStore) GetStatusReport(ctx context.Context) (job.Stats, error) {
	var stats job.Stats
	...
	FROM jobs`,
		job.StatusPending, job.StatusRunning, job.StatusCompleted, job.StatusFailed).Scan(
		&stats.Pending,
		&stats.Running,
		&stats.Completed,
		&stats.Failed,
...
func (s *PostgresStore) MarkCompleted(ctx context.Context, jobID string, result string) error {
	dbResult, err := s.db.ExecContext(ctx, `
		UPDATE jobs
		SET status = $1, updated_at = NOW(), result = $3, error_message = NULL, completed_at = NOW()
